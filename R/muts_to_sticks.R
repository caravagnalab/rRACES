...
#' @param cell_labels Table annotating the sticks (it can be the output of
#'  `get_events_table()`).
...
muts_to_sticks = function(phylo_forest, seq_results, cell_labels = NULL,
                          mutation_labels = NULL) {

  sampled_snvs <- phylo_forest$get_sampled_cell_mutations() %>%
    dplyr::as_tibble() %>%
    dplyr::select(-cell_id) %>% unique() %>%
    mutate(id = paste0("chr",chr,":",chr_pos,":",ref,":",alt))
  ...
  seq_results <- seq_results %>% dplyr::as_tibble() %>%
    dplyr::rowwise() %>%
  ...
  if (!is.null(cell_labels)) {
    map <- dplyr::full_join(seq_results, cell_labels, by = "cell_id") %>%
      filter(!is.na(chr))
  } else if (!is.null(mutation_labels)) {
    map <- dplyr::full_join(seq_results, mutation_labels, by = "id") %>%
      filter(!is.na(chr))
  } else {
    map <- seq_results
  }
