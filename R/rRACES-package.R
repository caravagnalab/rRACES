...
.onAttach <- function(libname, pkgname) {
    ...
    setMethod("+", signature(e1 = "Rcpp_Expression", e2 = "numeric"), function (e1, e2) {
        logics_sum(e1,e2)
    }, where=.GlobalEnv)
    setMethod("+", signature(e1 = "numeric", e2 = "Rcpp_Expression"), function (e1, e2) {
        logics_sum(e1,e2)
    }, where=.GlobalEnv)
    setMethod("+", signature(e1 = "Rcpp_Expression", e2 = "Rcpp_Variable"), function (e1, e2) {
        logics_sum(e1,e2)
    }, where=.GlobalEnv)
    setMethod("+", signature(e1 = "Rcpp_Variable", e2 = "Rcpp_Expression"), function (e1, e2) {
        logics_sum(e1,e2)
    }, where=.GlobalEnv)
    setMethod("+", signature(e1 = "Rcpp_Expression", e2 = "Rcpp_Expression"), function (e1, e2) {
        logics_sum(e1,e2)
    }, where=.GlobalEnv)
    ...
    setMethod("&", signature(e1 = "Rcpp_Formula", e2 = "Rcpp_Formula"), function (e1, e2) {
        logics_and(e1,e2)
    }, where=.GlobalEnv)
    setMethod("|", signature(e1 = "Rcpp_Formula", e2 = "Rcpp_Formula"), function (e1, e2) {
        logics_or(e1,e2)
    ...
    if (nrow(species) == 0) {
        ...
        for(s in my_tab$species)
        {
            ...
            if(with_epigenetics)
            sprintf(
                paste0('\n\tSpecies [%s]: %', nch, 's (deaths), %', nch, 's (duplications) and %', nch, 's (switches)'),
                s,
                s_ftab$fired[1],
                s_ftab$fired[2],
                s_ftab$fired[3]
                ...
                sprintf(
                paste0('\n\tSpecies [%s]: %', nch, 's (deaths) and %', nch, 's (duplications)'),
                s,
                s_ftab$fired[1],
                s_ftab$fired[2]
                ) %>% cat()
