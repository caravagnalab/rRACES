...
rRACES/RACES simulate the tumour growth according to two alternative models: 
...
homogeneous growth model by assigning the `Simulation`'s `duplicate_internal_cells` 
Boolean field. By setting it to `TRUE`, the homogeneous-growth model is used. If, 
instead, it is set to `FALSE`, the simulation evolves according with the 
"border"-growth model.
...
We now build a simulation and use the "border" growth model.

```{r}
sim <- new(Simulation, "Border Growth")

# Setting the "border" growth model is not needed as the
# boder growth model is the default.
