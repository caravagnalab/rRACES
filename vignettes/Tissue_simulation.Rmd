...
## Specie sand their evolutionary relations
...
<!-- To define an evolutionary relation between the genotypes `A` and `B`, we can set a parent/child relation that will fire at a predefined simulation time. This will mean that, a cell of genotype `A` will be randomly sampled and mutated to a cell of type `B`; the epistate is randomly chosen and maintained after mutation. -->

<!-- For example, we program an `A`-to-`B` timed transition from at time `30`. -->
...
# For example, total number of the deaths on `B+` at the end of the previous calls of the `run_to_*` methods
sim$get_firing_history() %>%
...
this history, independently of the

actual data stored after the evaluation of a `run_to_*` method. To do
this, we must set the `Simulation` property `history_delta` (default
$1$). By default, `history_delta` is set to $0$, but it can be set to
any positive real value $d$ so to sample the simulation statistics every
$d$ time units.
