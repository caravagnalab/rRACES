...
<<<<<<< Updated upstream
As above, the finer time-series can be depicted by using `plot_timeseries`.
=======
The time-series can be plot using `plot_timeseries`, or similary we can plot a Muller plot using [ggmuller](https://cran.r-project.org/web/packages/ggmuller).
...
```{r echo=FALSE, results='hide', message=FALSE, warning=FALSE}
unlink(sim_ts$get_name(), recursive = TRUE)
```
