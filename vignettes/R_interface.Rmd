...
unlink("some_folder", recursive = TRUE)
```
...
x <- create_simulation(
  ...
  tissue_size = c(400, 400)     # Size of the tissue
)
```
...
```{r, eval=FALSE}
# Program a timed transition from genotype "A" to genotype "B" at time 60
...
Ap_count = x$simulation$get_counts() %>% 
  dplyr::filter(genotype == "A", epistate == '+') %>% 
  dplyr::pull(counts)
...
x = run(x, what = list(genotype = 'A', epistate = '+', size = Ap_count + 1500))
...
Ap_switches = x$simulation$get_firings() %>% 
  dplyr::filter(genotype == "A", epistate == '+', event == 'switch') %>% 
  dplyr::pull(fired)

print(Ap_switches)

# Run until we get more of A+ switching to A-
x = run(x, what = list(genotype = 'A', epistate = '+', event = 'switch', count = Ap_switches + 1000))

# Check that we have more Ap_switches
x$simulation$get_firings() %>% 
  dplyr::filter(genotype == "A", epistate == '+', event == 'switch') %>% 
  dplyr::pull(fired)
