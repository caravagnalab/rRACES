...
Let us consider the __"Border Growth"__ simulation peformed in
...
Each of these species has a mutation rate that must be declated to 
the mutation engine before labeling the samples forest.
Let 3e-9, 6e-9, 4e-9, and 5e-9 be the mutation rates of the
species `A+`, `A-`, `B+`, and `B-`, respectively.

The two mutants may also be genetically characterized by some mutations. 
The mutations associated with each of the mutants must occur 
in any cell belonging to the mutant itself. Hence, also the mutant
mutations must be declared to the mutant engine before the labeling.

The method `MutationEngine$add_mutant` takes care of all these 
declarations.

For the sake of example, let us assume that `A` is characterized by a
mutation on chromosome 22, while `B` has two mutations on the same
chromosome.
...
# 1e-9 and 3e-8, respectively.
m_engine$add_mutant(mutant_rates = "A",
                    species_rates = c("+" = 1e-9, "-" =  3e-8),
                    mutant_SNVs = c(SNV("22", 10510210, "TAC", "T")),
                    mutant_CNAs = c())

# add the mutant "B" characterized by two SNVs on chromosome 22 (no CNA) and
# two epigenetic states. Its species "B+" and "B-" have mutation rate 8e-7
# and 5e-8, respectively.
m_engine$add_mutant("B", c("+" = 8e-7, "-" = 5e-8),
...
The probability for a SNV to occur depends on both its context and the 
active set of SBS coefficients. Thus, we need to specify a
default set of SBS coefficients (see [@alexandrov2020repertoire]).
...
an activation time, i.e., the new set of coefficients will be used
from the specified simulation time.

```{r}
# add a new set of SBS coefficients that will be used from simulated
# time 100 up to the successive SBS coefficient change.
m_engine$add_coefficients(time = 100,
...
```{r, eval = FALSE}
# build a SNV that occurs in one of the sampled cells
snv <- SNV("22", 13085324, "TTT", "G")

# get the identifier of the oldest cell in which it occurs
phylo_forest$get_first_occurrence(snv)
```
