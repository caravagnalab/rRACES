...
rRACES gives the chance to apply the germline mutations of one of 
the subjects available in the germline data provided when the 
mutation engine is built. This feature allows users to simulate 
the evolution of a specific cancer type on an individual having the 
...
The column `sample` contains the name of the available subjects.
The columns `pop` and `super_pop` report the population and 
the super population codes of the subjects. The last column, 
`gender`, contains the subject gender.

The method `MutationEngine$get_population_descritions()` clarifies the
meaning of the codes reported in the `pop` columns.
...
the subject whose genome used by rRACES as wild-type genome.

```{r}
m_engine$get_active_germline()
```

In order to change the germline subject, we must use the 
method `MutationEngine$set_germline_subject()`.

When a subject is selected for the first time, rRACES builds
a binary representation of the subject genome and it saves 
it for future use. This step may take few minutes, however,
all the successive selection of the same subject directly
loads the binary file.

```{r}
m_engine$set_germline_subjects(subjects[2,"sample"])
