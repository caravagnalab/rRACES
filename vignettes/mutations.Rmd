...
m_engine$add_mutant(mutant_name = "A",
                    ...
                    driver_SNVs = list(SNV("22", 10510210, "C", allele = 1)),
                    driver_CNAs = list(CNA(type = "A", "22",
...
In the above code, the driver CNAs and SNVs are declared by calling
the functions `CNA()` and `SNV()`, respectively. These function allow to specify
the allele in which the mutation must lay.

As far as the SNVs concern, rRACES provides the users with a more compact and,
sometimes, convenient approach. The mutation engine stores a list of known
driver mutations and labels each of them by a code which can be used
...
m_engine$get_known_drivers() %>% filter(chr == "chr22")
```

The code of the known driver mutations can be used in place of the full
SNV specification as follows.
...
test_infinite_sites_model <- function(phylo_forest) {
  ...
  if (length(first_occurrences) >= 2) {
    ...
    paste0("SNV('", snv$get_chromosome(), "'',",
           snv$get_position_in_chromosome(), ",'", snv$get_ref_base(),
           "','", snv$get_alt_base(),
