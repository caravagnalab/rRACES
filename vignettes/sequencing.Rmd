...
of samples in the phylogenetic forest. The first four columns
describe the mutation and report the chromosome and the position
in the chromosome of the mutation (columns `chr` and
...
The sequencing simulation supports cell partition by epigenetic
state (epi-FACS) using the `simualate_seq()` parameter `epi_FACS`.
At the beginning of the sequencing simulation process, each
sample in the phylogenetic forest is split into two distinct
samples: `<sample name>_P`, which contains all the cells in the
original sample having epigenetic state `+`, and `<sample name>_N`,
that consists in the cells having epigenetic state `-`.
The result of the simulation reports the data for the split samples.
...
and that of all the sample cells, either tumour or normal.
...
way, rRACES automatically gathers information about wild-type cells,
and it also simulates the sequencing of normal cells when the SAM
writing is enabled.
...
no_error_seq <- new(ErrorlessIlluminaSequencer)
