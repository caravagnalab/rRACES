...
```{r}
# Program a timed transition from "A" to "B" at time 20
sim$add_timed_mutation("A", "B", 20)
...
# Run the simulation up until the there are less than 500 cells in the species A+
sim$run_up_to_size("A+", 500)

# Counts per species reports now A+ equal to 500
sim$get_counts()
```

```{r}
Amd_firings = sim$get_firings() %>% 
  dplyr::filter(event == 'death', genotype == 'A', epistate == '-') %>% 
  dplyr::pull(fired)

# Run the simulation up until number of deaths in A- is at least 100
sim$run_up_to_event("death", "A-", Amd_firings + 50)

# Get the number of fired event per species. The row "death", "A", "-" reports 50 more values
sim$get_firings()
```

```{r}
# Run the simulation 15 more units of time
