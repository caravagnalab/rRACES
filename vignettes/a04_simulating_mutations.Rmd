...
`vignette("a03_growth_models")`. It involves the two mutants `A` and
...
mutation on chromosome 22 on position 10510270, while `B` has two
mutations on the same chromosome in position 12028576 and 12029660.
...
m_engine$add_mutant(mutant_rates = "A",
                    species_rates = c("+" = 3e-9, "-" = 6e-9),
                    mutant_SNVs = c(SNV("22", 10510210, "TAC", "T")),
                    mutant_CNAs = c())

# add the mutant "B" characterized by two SNVs on chromosome 22 (no CNA) and
# two epigenetic states. Its species "B+" and "B-" have mutation rate 4e-9
# and 5e-9, respectively.
m_engine$add_mutant("B", c("+" = 4e-9, "-" = 5e-9),
...
simulation peformed in `vignette("a03_growth_models")`,
