...
The function `build_mutation_engine()` builds the mutation engine and
these two indices.

## Customizing the mutation engine

While the most convient way to invoke `build_mutation_engine()` is by using
the parameter `setup_code` (see `vignette["mutations"]`), the are cases
in which a custom set-up is required. In these cases,
the function `build_mutation_engine()` can be called by specifying the name
...
function `build_mutation_engine()` builds
...
`build_mutation_engine()` allows to sample the reference genome contexts and
...
be specified during the `build_mutation_engine()` call.
...
To mitigate the memory requirement, the function `build_mutation_engine()`
