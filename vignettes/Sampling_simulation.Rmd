...
For this simulation, we define two samples as if we performed multi-region sequencing. 
...
sim$sample_cells(bottom_left = c(400, 400), top_right = c(480, 480))

# Bounding box from (550, 550) to (630, 630)
sim$sample_cells(bottom_left = c(550, 550), top_right = c(630, 630))
```
...
Again, we sample twice on the same locations as before (which have been re-filled with cells).
