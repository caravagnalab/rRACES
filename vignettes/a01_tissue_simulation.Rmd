...
In order to save the simulation progresses in the disk and recover 
it in the future, use the optional Boolean parameter
`save_snapshots`. By setting `save_snapshots` to `TRUE`, the 
simulation progresses will be saved in a directory whose name is 
...
sim <- new(Simulation, save_snapshots = TRUE)
```
